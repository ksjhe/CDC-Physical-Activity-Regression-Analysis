...
We first see if there is a linear relationship between at least one of the explanatory variables to the response by running a summary of the model. The resulting p-value from the significance of linear regression test is =< 0.05, so we conclude that the response is linearly related to at least one of the explanatory variables.
